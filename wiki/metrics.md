...
管理页面可以导出最近 100 条或当前日志上限内的全部用量记录。导出文件中的 `records`
按时间正序排列。`POST /api/admin/metrics/import` 接受该导出对象或 records 数组，以请求
`id` 去重；已有记录和同一文件内的重复记录不会重复计入，导入后会从保留明细重建聚合统计。
