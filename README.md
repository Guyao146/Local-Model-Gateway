...
- 错误消息前缀：最长 60 个字符，默认为空。填写后所有返回给客户端的错误消息（如 400/401/404/429/502 及上游错误透传）都会加上该前缀，例如填写 `[SakuraGateway]` 后客户端会看到 `[SakuraGateway] model 不能为空`，便于区分错误来自网关还是上游；成功响应和流式数据不受影响
