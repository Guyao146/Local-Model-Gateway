...
后台“本地模型选择”区域会按“上游 → 模型 ID 前缀”两级分组展示模型。所有上游和前缀默认收起。前缀取模型 ID 中第一个 `-`、`_`、`:` 或 `/` 之前的部分，例如 `gpt-4o` 归入 `gpt`、`claude-3-7-sonnet` 归入 `claude`。可以：

- 点击“拉取全部模型”尝试从所有启用上游的 `/v1/models` 获取模型；不提供该接口的 Anthropic 站点继续使用上游表单中的手工模型
- 使用搜索框按站点名、模型 ID 或显示名筛选
- 分别点击上游或前缀标题展开、收起，也可以使用顶部“全部展开/全部收起”同时控制两层
- 按整个上游“全部勾选/全部取消”
- 按单个模型前缀“全部勾选/全部取消”
- 勾选要暴露给本地 APP 的模型
