...
后台的“请求统计”区域会显示请求总数、成功率、故障转移次数、输入/输出/总 Token、按上游统计以及请求元数据日志。日志默认保留最近 5000 条（可用环境变量 `LOCAL_MODEL_GATEWAY_MAX_LOGS` 调整，最少 100），页面先显示最新 100 条，点击“加载更多”按 100 条为一页向下翻。日志不会保存请求内容、请求头、API Key 或上游完整地址。可以在后台点击“清空统计”。聚合计数保存在 `data/metrics.json`，请求日志以追加写入的方式保存在 `data/metrics-log.jsonl`（旧版内联日志会在首次启动时自动迁移），与上游配置分开保存。
